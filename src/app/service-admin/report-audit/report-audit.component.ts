...
export class ReportAuditComponent implements OnInit {
  ...
  barChartData:any=[];
    reportSide= [ 
      ...
      {"name": "MR for Prevent "}, ];
      dateReport= ["Today","This week","This Month","This Year","Over All"];
      moreReport=  [ 
      ...
      options: {
        ...
        public mbarChartLabels:string[] = ['Audit Service Reports'];
...
var year = new Date(new Date().getFullYear() , 0, 1);
    ...
    if(this.Admin_check == null){
      ...
      this._api.audit_down_chart(this.data).subscribe((response: any) => {
        ...
        this.total_coutn = this.rows.total_coutn;
        ...
        this.barChartData = [
          ...
          {data:[this.rows.total_coutn], label:'Over all Job'},
...
if(this.reportsrange=='Over All'){
  let dates = new Date('2022-01-01');

  this. data = {
    "start_date":new DatePipe('en-US').transform(dates,'yyyy-MM-dd') ,
    "end_date": new DatePipe('en-US').transform(this.end,'yyyy-MM-dd'),
    "user_type" : "",
    "access_location":[],
  }
...
this._api.audit_down_chart(this.data).subscribe((response: any) => {
 ...
 clickdata(e:any,data) {
  ...
  sessionStorage.setItem("chart_page",'audit');
