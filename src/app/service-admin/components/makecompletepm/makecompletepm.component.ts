...
@Component({
  selector: 'app-makecompletepm',
  templateUrl: './makecompletepm.component.html',
  styleUrls: ['./makecompletepm.component.css']
})
export class MakecompletepmComponent implements OnInit {
  final_data = [];
  search:any;
  searchQR : any;
  S_Date:any;
...
JOB_END_TIME : ""+new Date(),
   ...
   this._api.breakdown_mark_complete(a).subscribe((data:any)=>{
  ...
  submit(){
    ...
    this._api.preventive_date_wise_filter(data).subscribe((data:any)=>{
     this.rows=data['Data'];
