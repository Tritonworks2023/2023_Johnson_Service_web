...
@Component({
  selector: 'app-makecompletead',
  templateUrl: './makecompletead.component.html',
  styleUrls: ['./makecompletead.component.css']
})
export class MakecompleteadComponent implements OnInit {
  final_data = [];
  search:any;
  searchQR : any;
  S_Date:any;
...
JOB_END_TIME : ""+new Date(),
   ...
   this._api.audit_mark_complete(a).subscribe((data:any)=>{
  ...
  submit(){
    ...
    this._api.audit_date_wise_filter(data).subscribe((data:any)=>{
     this.rows=data['Data'];
